 ...
 * Last file update: 19-Oct-2018 21:13 (eg)
 *
 */
#include <ctype.h>
#include "stklos.h"
...
static void print_cycle(SCM exp, SCM port, int mode, cycles *c)
{
  ...
  tmp =  STk_assv(exp, c->seen);
  value =  STk_hash_ref_default(c->hseen, exp, STk_void);
  if (INTP(value)) {
    // value= CDR(tmp);
...
Top:
  ...
  tmp = STk_assv(exp, c->seen);
  if ((STk_hash_ref_default(c->hseen, exp, STk_void)) == STk_void) {
    /* We have never seen this cell so far */
    STk_hash_set(c->hseen, exp, STk_false);
    c->seen = STk_cons(STk_cons(exp, STk_false), c->seen);
    ...
    STk_hash_set(c->hseen, exp, STk_true);
    CDR(tmp) = STk_true;
  }
}
...
static void pass2(SCM exp, SCM port, int mode, cycles *c)
{
  if (!CONSP(exp) && !VECTORP(exp))
    ...
    tmp = STk_assv(exp, c->seen);
    if (STk_hash_ref_default(c->hseen, exp, STk_void) == STk_true) {
      /* First use of this label. Assign it a value */
      (value=CDR(tmp));
      STk_fprintf(port, "#%d=", c->label);
      CDR(tmp) = MAKE_INT(c->label);
      STk_hash_set(c->hseen, exp, MAKE_INT(c->label));
      c->label++;
    }
...
void STk_print_star(SCM exp, SCM port, int mode)
{
  ...
  c.hseen = STk_make_basic_hash_table();
  c.seen = STk_nil;
  c.label = 0;

  pass1(exp, &c);
  STk_hash_stats(c.hseen, NULL);
  pass2(exp, port, mode, &c);
}
