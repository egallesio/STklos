...
"%%require4syntax" " "
"%do-include" " "
"(lambda (kind files) (let ((inc (string->symbol (format \"%%~a\" kind)))) (if (null? files) (error kind \"at least one parameter must be provided\") (let* () (when #t (%debug \"On devrait ajouter ~S à path\" \"/tmp/bar\")) `(,inc ,@(map (lambda (x) (or (find-path x (cons \"/tmp/bar\" *load-path*)) x)) files))))))" " "
"\"%%~a\"" " "
"\"at least one parameter must be provided\"" " "
"\"On devrait ajouter ~S à path\"" " "
"\"/tmp/bar\"" " "
"\"/tmp/bar\"" " "
