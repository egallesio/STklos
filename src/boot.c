...
"time" " "
"(lambda args (let ((time-start (gensym)) (res (gensym)) (counting? (gensym)) (allocs (gensym)) (bytes (gensym))) `(let ((,counting? (%count-allocations))) (%thread-allocations-reset!) (%count-allocations #t) (let* ((,time-start (exact-clock)) (,res (begin ,@args)) (,allocs (%thread-allocation-counter)) (,bytes (%thread-allocation-bytes))) (eprintf \"Elapsed time: ~S ms\\n\" (/ (- (exact-clock) ,time-start) 1000.0)) (eprintf \"Allocations: ~S byte~A in ~S allocation call~A\\n\" ,bytes (if (> ,bytes 1) \"s\" \"\") ,allocs (if (> ,allocs 1) \"s\" \"\")) (%count-allocations ,counting?) ,res))))" " "
"%count-allocations" " "
"%thread-allocations-reset!" " "
"exact-clock" " "
"%thread-allocation-counter" " "
"%thread-allocation-bytes" " "
"\"Elapsed time: ~S ms\\n\"" " "
"1000.0" " "
"\"Allocations: ~S byte~A in ~S allocation call~A\\n\"" " "
"\"s\"" " "
"\"\"" " "
"\"s\"" " "
"\"\"" " "
