...
"time" " "
"(lambda args (let ((time-start (gensym)) (res (gensym)) (alloc-bytes-start (gensym)) (alloc-objs-start (gensym)) (alloc-times-start (gensym)) (alloc-bytes-end (gensym)) (alloc-objs-end (gensym)) (alloc-times-end (gensym))) `(let* ((,time-start (clock))) (let-values (((,alloc-bytes-start ,alloc-objs-start ,alloc-times-start) (%thread-allocations (current-thread)))) (let ((,res (begin ,@args))) (let-values (((,alloc-bytes-end ,alloc-objs-end ,alloc-times-end) (%thread-allocations (current-thread)))) (format (current-error-port) \"Elapsed time: ~S ms\\n\" (- (clock) ,time-start)) (format (current-error-port) \"Allocated: ~S bytes (~~ ~S Scheme objects) in ~S allocation calls\\n\" (- ,alloc-bytes-end ,alloc-bytes-start) (- ,alloc-objs-end ,alloc-objs-start) (- ,alloc-times-end ,alloc-times-start)) ,res))))))" " "
"let-values" " "
"%thread-allocations" " "
"current-thread" " "
"\"Elapsed time: ~S ms\\n\"" " "
"\"Allocated: ~S bytes (~~ ~S Scheme objects) in ~S allocation calls\\n\"" " "
"tagbody" " "
