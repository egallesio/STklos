...
(define (test-sphere sphereg dim-sizes REPS rotate?)
  ...
  (define (check-zero samples)
    ...
    (test "srfi-194-sphere" #t (< zz num-stddev)))
...
(define (test-ball ballg dim-sizes)
  ...
  (define (test-ellipsoid a b N)
    ...
    (test "srfi-194-sphere" #t (every (lambda (p)
    ...
    (for-each (lambda (p m)
                ...
                (test "srfi-194-sphere" #t (< (abs (- (* p N) m))
                                (* STDs (sqrt (* N p (- 1 p)))))))
              (list p-right p-top)
  ...
  (define (test-ellipse a b N)
    ...
    (test "srfi-194-sphere" #t (every in-ellipse? interior-points))
