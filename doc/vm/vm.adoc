...
* `(%vm-collecting-stats?)` returns true when the system is collecting
  statistics (that is, when the variable `collect_stats` is one -- see
  `vm.c`), and false otherwise.
* `(%vm-collect-stats-start)` and `(%vm-collect-stats-stop)` will turn
...
There is also one primitive that can be used to determine if the
system has been compiled with the profiling code: `(%vm-has-stats)`
