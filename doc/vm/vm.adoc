...
`PREPARE_CALL` pushes an activation record on the stack. The `INVOKE`
opcodes call procedures – local or global; in tail position or not.
...
In the next example, `GREF-INVOKE` is called with arguments 0 and 2. The
value 2 is the address of the procedure in the stack. The `IN-SINT-ADD2`
procedure is called afterwards to sum 1 with the return from `f`.
