...
1. Fetch the index of the global variable
...
5. **Patch the code**, that is, changing the `GLOBAL_REF` or `GLOBAL_SET` insrtuction
   into a `UGLOBAL_REF` or `UGLOBAL_SET` ('U' prefix here is for already used vrariable)
...
*And*, of course, the `n`-th element of the table contains the value
...
* `RELEASE_LOCK` will release the lock, regardless of the thread having it
  or not. The flag indicating ownership by this thread is cleared.
