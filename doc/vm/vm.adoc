...
. Copies the Scheme and C stacks into the continuation object;
. Calls `patch_environment(vm)`. This will clone the environments
   ...
   values must be copied to the ones at the time the continuation was captured);
...
. Uses the usual `setjmp` method to either return the continuation
