...
When the code has been compiled as this, the VM will, at the end of each iteration,
check the instruction that was just executed and update:
...
Five Scheme primitives are then available:
...
There is also one primitive that can be used to determine if the system has been compiled with the profiling code:
`(%vm-has-stats)` will be either true or false.
...
So the macro will return the same values that the coude would, and will profile only
