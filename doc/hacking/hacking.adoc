...
The `#%if` to which it exapands into is an *internal compiler call*
(named `icall` in STklos). It looks like a Scheme symbol, but symbols
cannot start with the sharp sign (`#`), so one cannot use this as a symbol name:
...
However, `icall` objects are self-evaluating, just like symbols:
...
* a "function" (actually the Scheme procedure that compiles this form, just
  like the others in `compiler.stk`). The function field may be set to `#f`.
...
We now implement a special form `twice`, that compiles the code inside it twice.

[source,scheme]
----
stklos> (define-macro (twice . args)            `(#%twice ,@args))
----

We *do not* need to "create" the `#%twice`  icall. We just create the function that will
compile it, *inside the `STKLOS-COMPILER` module*:

[source,scheme]
----
stklos> (select-module STKLOS-COMPILER)
STKLOS-COMPILER>
(define (compile-%%twice args env tail?)
  (if (null? (cdr args))
      (emit 'IM-VOID)      ;; (twice) => void
      (compile
       `(begin ,@(cdr args)  ;; once
               ,@(cdr args)) ;; twice
       env args tail?)))
...
And that's all. We can test our new special form (and we move out of
the `STKLOS-COMPILER` module, since it's not necessary anymore):

[source,scheme]
----
STKLOS-COMPILER> (select-module stklos)
stklos> (disassemble-expr '(twice (print 1)))
...
The code was compiled twice!

[source,scheme]
----
stklos> (twice (print 1))
1
1
----

Of course, STklos will optimize out whatever redundancies we introduce, so
`(twice 1)` will generate the same code as `(begin 1)`.

Now we illustrate the use of `%icall-function`.

[source,scheme]
----
stklos> (define-macro (double . args)            `(#%double ,@args))
----

We may have an `icall` object with a different name from its compiling function. The
procedure `%icall-function-set!` allows one to define the compiling procedure for
that special form.

[source,scheme]
----
stklos> (%icall-function #%double)
#f
stklos> (%icall-function-set! #%double (in-module STKLOS-COMPILER compile-%%twice))
stklos> (%icall-function #%double)
#[closure compile-%%twice]
stklos> (double (print 2))
2
2
----
