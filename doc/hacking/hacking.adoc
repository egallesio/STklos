...
* Add a line describing it in `etc/srfis.stk` (the format is described
in the file itself).
* Tests reside in the `tests` directory. Add the tests in a file
`tests/srfis/9999.stk`

For new SRFIs, adding its description in `etc/srfis.stk` suffices to
update
...
In Scheme, the 'static scope' at a certin position of the code text is
the bindings of variables to locations that can be inferred from the
program. For example,
...
(let ((number 2)
      (text "something")
      (more-text "more things"))
  (if (number > 0)
      (let ((number -1))
        (display "x""))))
----
In the line "display x", the static scope is
...
The *dynamic scope* is the way the virtual machine implements the static scope.
It will create a structure that is a linked list of _frames_ (each frame is
a sequence of locations; but variable names are not stored anymore -- the
compiler translated them all into locations already).

The data type 'environment' contains thestatic environment, the dynamic
environment, and the module for a iven point in the code.

[source,c]
----
struct environment_obj {
  stk_header header;
  SCM dyn_env;                /* dynamic (runtime) environment - a LIST */
  SCM static_env;             /* static (compiler) environment - */
  SCM module;                 /* dynamic module */
};
----

The following primitives are used to work with environments:
...
* `%environment-module`
Along with the compiler call `#%current-environment'.
...
(%environment-module e)              => #[module stklos]
(%environment-static-environment e)  => (((b) ()) ((a) ()))
(%environment-dynamic-environment e) => #[frame 7f1bbf886420]
----
That frame as a "next frame" below it. This is clear from the
definition of frame in `stklos.h`:
...
So `#%current-environment` will, *at runtime*, capture both the dynamic (runtime)
environment and the static (compile time) environment.
