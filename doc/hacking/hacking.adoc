...
// Last file update:  4-Sep-2022 19:18 (eg)
...
* `STk_getc(SCM port)` for printing a single character
* `STk_get_character(SCM port)` for printing a single character (result may be a wide char)
...
reading there is also `STk_input`. These standard ports are defined in
