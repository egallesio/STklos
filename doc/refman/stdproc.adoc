...
NOTE: Syntax for reading bytevectors (`#u8`), which are {{rseven}}, is always
available. However, various SRFIs or the permit to manipulate several types of
uniform vectors or the {{rseven}}-large `(scheme vector)` library propose a
lexical syntax for those vectors. See the documentation of the
