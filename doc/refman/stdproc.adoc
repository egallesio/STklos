...
// Last file update:  6-Jun-2022 16:40 (eg)
...
representation which consists of two integers representing the
number of seconds and nanoseconds elapsed since the *_Epoch_*
(00:00:00 on January 1, 1970, Coordinated Universal Time --UTC).
Dates can also be represented with date structures.
SRFI 19 contains more time and date-related procedures,
such as time durations, tome comparisons and Julian calendar
functions.
...
{{insertdoc 'time?}}
{{insertdoc 'time-type}}
{{insertdoc 'time-second}}
{{insertdoc 'time-nanosecond}}
