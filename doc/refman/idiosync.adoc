...
==== STklos reader features

When reading inexact real numbers in base 10, {{stklos}} will interpret
the `#` sign as a zero, except when it is the first character. If the
exactness is not specified, it is automatically forced to inexact.
...
{{stklos}} accepts both `(...)` and `[...]` as list delimiters, but they
cannot be mixed:
